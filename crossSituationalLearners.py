...
def compute_total_activation(matrix, outcome, trial_cues, lam):
    ...
    :param lam:         the maximum amount of activation an outcome can sustain from all the afferent cues
    ...
    if v_total > lam:
        ...
        print(v_total)
        raise ValueError('Something went wrong! The total amount of activation cannot exceed %d: ' % lam +
...
def update_outcome_weights(matrix, outcome, trial_cues, trial_outcomes, lam, ab):
    ...
    l = lam if outcome in trial_outcomes else 0
    delta_v = ab * (l - v_total)
    ...
    for cue in trial_cues:
        ...
        if matrix[outcome][cue] > lam:
            print(outcome, cue, matrix[outcome][cue])
            raise ValueError('An outcome can only sustain an activation of %d and it looks like the ' % lam +
...
def discriminative_learner(training_trials, iterations, alpha=0.1, beta=0.1, lam=1,
    ...
    for i in range(iterations):
        ...
        for trial in training_trials:
            ...
            for outcome in outcomes:
                ...
                ith_associations = update_outcome_weights(ith_associations, outcome, trial_cues,
                                                          trial_outcomes, lam, ab)
        ...
        for trial in training_trials:
            ...
            if outcome not in ith_hypotheses:                               # IF it is new
                ...
                if outcome not in ith_recalled:                               # BUT encountered just once
                    ...
                    k = list(ith_recalled[outcome].keys())[0]                   # find how many times the cue-outcome
                    if len(alpha_1) >= ith_recalled[outcome][k]:                     # hypothesis was already retrieved
                        p = alpha_1[ith_recalled[outcome][k]-1]                      # recall with increased alpha
                    else:
...
def cross_situational_learning(in_file, i=200, s=6, print_output=False):

    """
    :param in_file:          a .txt file containing two columns. The first column is assumed to contain cues, while
                                the second columns is assumed to contain outcomes.
    :param i:              an integer stating how many iterations need to be performed to evaluate each learner
    :param s:                an integer to seed the random permutation of learning trials in the training set, for
                                reproducibility
    :param print_output:        a boolean specifying whether average associations should be printed after having been
                                computed
    ...
    training_trials = read_input_trials(in_file)

    outcomes = defaultdict(dict)

    outcomes['ndl'] = discriminative_learner(training_trials, i, seed=s, print_output=print_output)
    outcomes['HebbianLearner'] = hebbian_learner(training_trials, print_output=print_output)
    outcomes['probabilisticLearner'] = probabilistic_learner(training_trials, i, seed=s, print_output=print_output)
    outcomes['HTM'] = hypothesis_testing_model(training_trials, i, print_output=print_output)
...
def main():
    ...
    if not os.path.exists(args.input_file):
        ...
        learning_outcomes = cross_situational_learning(args.input_file, i=args.n_iter,
                                                       s=args.seed, print_output=args.out)
